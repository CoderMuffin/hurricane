#include <taylor_math.h>
#include <hurricane/fixed.h>

/// Writes the three xyz parameters to the specified output vector.
void hc_vec3_set(fixed x, fixed y, fixed z, fixed out[3]) {
  out[0] = x;
  out[1] = y;
  out[2] = z;
}

/// Copies vector from to vector to.
void hc_vec3_copy(const fixed from[3], fixed to[3]) {
  to[0] = from[0];
  to[1] = from[1];
  to[2] = from[2];
}

/// Adds two vectors, storing the result in out.
void hc_vec3_add(const fixed a[3], const fixed b[3], fixed out[3]) {
  out[0] = a[0] + b[0];
  out[1] = a[1] + b[1];
  out[2] = a[2] + b[2];
}

/// Subtracts two vectors, storing the result in out.
void hc_vec3_sub(const fixed a[3], const fixed b[3], fixed out[3]) {
  out[0] = a[0] - b[0];
  out[1] = a[1] - b[1];
  out[2] = a[2] - b[2];
}

/// Multiplies all components of a vector by a scalar, storing the result in out.
void hc_vec3_mul(const fixed a[3], const fixed b, fixed out[3]) {
  out[0] = fm(a[0], b);
  out[1] = fm(a[1], b);
  out[2] = fm(a[2], b);
}

/// Multiplies two vectors, storing the result in out.
void hc_vec3_vmul(const fixed a[3], const fixed b[3], fixed out[3]) {
  out[0] = fm(a[0], b[0]);
  out[1] = fm(a[1], b[1]);
  out[2] = fm(a[2], b[2]);
}

/// Divides all components of a vector by a scalar, storing the result in out.
void hc_vec3_div(const fixed a[3], const fixed b, fixed out[3]) {
  out[0] = fd(a[0], b);
  out[1] = fd(a[1], b);
  out[2] = fd(a[2], b);
}

/// Divides two vectors, storing the result in out.
void hc_vec3_vdiv(const fixed a[3], const fixed b[3], fixed out[3]) {
  out[0] = fd(a[0], b[0]);
  out[1] = fd(a[1], b[1]);
  out[2] = fd(a[2], b[2]);
}

/// Returns the square magnitude of a vector (useful if the sqrt calculation is not needed, as this saves unnecessary computation).
fixed hc_vec3_sqrmag(const fixed v[3]) {
  return fm(v[0], v[0]) + fm(v[1], v[1]) + fm(v[2], v[2]);
}

/// Returns the magnitude of a vector.
fixed hc_vec3_mag(const fixed v[3]) {
  return fx_sqrt(fm(v[0], v[0]) + fm(v[1], v[1]) + fm(v[2], v[2]));
}

/// Calculates the cross product of two vectors, storing the result in out.
void hc_vec3_cross(const fixed a[3], const fixed b[3], fixed out[3]) {
  out[0] = fm(a[1], b[2]) - fm(a[2], b[1]);
  out[1] = fm(a[2], b[0]) - fm(a[0], b[2]);
  out[2] = fm(a[0], b[1]) - fm(a[1], b[0]);
}

/// Normalizes a vector (sets its length to 1).
void hc_vec3_normalize(const fixed in[3], fixed out[3]) {
  fixed mag = hc_vec3_mag(in);
  if (tf(mag) == 0) {
    hc_vec3_copy(in, out);
  } else {
    hc_vec3_div(in, mag, out);
  }
}

/// Linearly interpolates between two vectors by f, storing the result in out.
void hc_vec3_lerp(const fixed a[3], const fixed b[3], fixed f,
                  fixed out[3]) {
  out[0] = fm(a[0], (tf(1.0) - f) + fm(b[0], f));
  out[1] = fm(a[1], (tf(1.0) - f) + fm(b[1], f));
  out[2] = fm(a[2], (tf(1.0) - f) + fm(b[2], f));
}

/// Returns the dot product of two vectors
fixed hc_vec3_dot(const fixed a[3], const fixed b[3]) {
  return fm(a[0], b[0]) + fm(a[1], b[1]) + fm(a[2], b[2]);
}
