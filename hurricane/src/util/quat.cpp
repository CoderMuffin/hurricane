...
#define QUATERNION_EPS 0.0_f01_f
...
void hc_quaternion_from_axis_angle(const fixed axis[3], const fixed angle,
  ...
  output->w = cos(angle / 2.0_f);
  fixed c = sin(angle / 2.0_f);
  output->v[0] = c * axis[0];
  output->v[1] = c * axis[1];
  output->v[2] = c * axis[2];
}

fixed hc_quaternion_to_axis_angle(hc_quaternion *q, fixed output[3]) {
  assert(output != NULL);
  // Formula from
  // http://www.euclideanspace.com/maths/geometry/rotations/conversions/hc_quaternionToAngle/
  fixed angle = 2.0_f * acos(q->w);
  fixed divider = sqrt(1.0_f - q->w * q->w);
...
void hc_quaternion_from_x_rotation(fixed angle, hc_quaternion *output) {
  assert(output != NULL);
  fixed axis[3] = {1.0_f, 0, 0};
  hc_quaternion_from_axis_angle(axis, angle, output);
}

void hc_quaternion_from_y_rotation(fixed angle, hc_quaternion *output) {
  assert(output != NULL);
  fixed axis[3] = {0, 1.0_f, 0};
  hc_quaternion_from_axis_angle(axis, angle, output);
}

void hc_quaternion_from_z_rotation(fixed angle, hc_quaternion *output) {
  assert(output != NULL);
  fixed axis[3] = {0, 0, 1.0_f};
  hc_quaternion_from_axis_angle(axis, angle, output);
}
...
hc_quaternion hc_quaternion_new_euler_zyx(fixed eulerZYX[3]) {
  ...
  fixed cy = cos(eulerZYX[2] * 0.5_f);
  fixed sy = sin(eulerZYX[2] * 0.5_f);
  fixed cr = cos(eulerZYX[0] * 0.5_f);
  fixed sr = sin(eulerZYX[0] * 0.5_f);
  fixed cp = cos(eulerZYX[1] * 0.5_f);
  fixed sp = sin(eulerZYX[1] * 0.5_f);
...
void hc_quaternion_to_euler_zyx(hc_quaternion *q, fixed output[3]) {
  ...
  fixed sinr_cosp = +2.0_f * (q->w * q->v[0] + q->v[1] * q->v[2]);
  fixed cosr_cosp = +1.0_f - 2.0_f * (q->v[0] * q->v[0] + q->v[1] * q->v[1]);
  output[0] = atan2(sinr_cosp, cosr_cosp);

  // Pitch (y-axis rotation)
  fixed sinp = +2.0_f * (q->w * q->v[1] - q->v[2] * q->v[0]);
  if (fabs(sinp) >= 1)
  ...
  // Yaw (z-axis rotation)
  fixed siny_cosp = +2.0_f * (q->w * q->v[2] + q->v[0] * q->v[1]);
  fixed cosy_cosp = +1.0_f - 2.0_f * (q->v[1] * q->v[1] + q->v[2] * q->v[2]);
...
fixed hc_quaternion_norm(hc_quaternion *q) {
  assert(q != NULL);
...
void hc_quaternion_normalize(hc_quaternion *q, hc_quaternion *output) {
  assert(output != NULL);
  fixed len = hc_quaternion_norm(q);
...
void hc_quaternion_rotate(const hc_quaternion *q, fixed v[3], fixed output[3]) {
  ...
  fixed ww = q->w * q->w;
  fixed xx = q->v[0] * q->v[0];
  fixed yy = q->v[1] * q->v[1];
  fixed zz = q->v[2] * q->v[2];
  fixed wx = q->w * q->v[0];
  fixed wy = q->w * q->v[1];
  fixed wz = q->w * q->v[2];
  fixed xy = q->v[0] * q->v[1];
  fixed xz = q->v[0] * q->v[2];
  fixed yz = q->v[1] * q->v[2];
...
void hc_quaternion_rotate_inverse(const hc_quaternion *q, fixed v[3], fixed output[3]) {
  ...
  result[0] = ww * v[0] + 2_f * wy * v[2] - 2 * wz * v[1] + xx * v[0] +
              2 * xy * v[1] + 2 * xz * v[2] - zz * v[0] - yy * v[0];
  result[1] = 2 * xy * v[0] + yy * v[1] + 2 * yz * v[2] + 2 * wz * v[0] -
              zz * v[1] + ww * v[1] - 2 * wx * v[2] - xx * v[1];
  result[2] = 2 * xz * v[0] + 2 * yz * v[1] + zz * v[2] - 2 * wy * v[0] -
...
void hc_quaternion_slerp(hc_quaternion *q1, hc_quaternion *q2, fixed t,
  ...
  fixed cosHalfTheta = q1->w * q2->w + q1->v[0] * q2->v[0] +
                        q1->v[1] * q2->v[1] + q1->v[2] * q2->v[2];

  // if q1=q2 or qa=-q2 then theta = 0 and we can return qa
  if (fabs(cosHalfTheta) >= 1.0_f) {
    hc_quaternion_copy(q1, output);
    return;
  }

  fixed halfTheta = acos(cosHalfTheta);
  fixed sinHalfTheta = sqrt(1.0_f - cosHalfTheta * cosHalfTheta);
  // If theta = 180 degrees then result is not fully defined
  // We could rotate around any axis normal to q1 or q2
  if (fabs(sinHalfTheta) < QUATERNION_EPS) {
    result.w = (q1->w * 0.5_f + q2->w * 0.5_f);
    result.v[0] = (q1->v[0] * 0.5_f + q2->v[0] * 0.5_f);
    result.v[1] = (q1->v[1] * 0.5_f + q2->v[1] * 0.5_f);
    result.v[2] = (q1->v[2] * 0.5_f + q2->v[2] * 0.5_f);
  } else {
    // Default hc_quaternion calculation
    fixed ratioA = sin((1 - t) * halfTheta) / sinHalfTheta;
    fixed ratioB = sin(t * halfTheta) / sinHalfTheta;
