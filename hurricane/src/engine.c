...
static fixed hc_internal_eye_dist = tf(400 / (2 * 0.7)); // 70 fov default
static fixed hc_internal_frame_tmp_vec[3];
static fixed camera_space[3];
...
void hc_world_to_screen(const hc_object *camera,
                        ...
                        const fixed p_world[3], int out[2], fixed *out_z) {
  hc_vec3_sub(p_world, camera->position, camera_space);
  hc_quaternion_rotate_inverse(&camera->rotation,
                               camera_space,
                               camera_space);

  out[0] = fd(fm(camera_space[0], hc_internal_eye_dist), camera_space[2]) + tf(renderer_config.width / 2);
  out[1] = fd(fm(camera_space[1], hc_internal_eye_dist), camera_space[2]) + tf(renderer_config.height / 2);
  *out_z = fd(tf(1.0), camera_space[2]);
}

fixed hc_internal_lighting(hc_object *object, int i) {
  fixed a[3], b[3], c[3], t1[3], t2[3], normal[3];
  ...
  return fd((hc_vec3_dot(VEC3(1, 0, 0), normal) + 1), tf(2));
}
void triangle(int x0, int y0, fixed z0, int x1, int y1, fixed z1,
...
void hc_render_object(hc_object *camera, hc_object *object) {
  ...
  for (int i = 0; i < object->geometry->face_count * 3; i += 3) {
    ...
    fixed dot = hc_internal_lighting(object, i);

    hc_internal_engine_renderer.triangle(a[0], a[1], az, b[0], b[1], bz, c[0],
                                         c[1], cz,
                                         ti(fm(tf(object->geometry->colors[i]), dot)),
                                         ti(fm(tf(object->geometry->colors[i + 1]), dot)),
                                         ti(fm(tf(object->geometry->colors[i + 2]), dot)));
  }
}

void hc_set_fov(fixed fov, hc_renderer_config renderer_config, bool use_height) {
  hc_internal_eye_dist = fd(
    tf(use_height ? renderer_config.height : renderer_config.width),
    fm(tf(2), fx_tan(fm(fov, tf(M_PI / 360)))));
}
